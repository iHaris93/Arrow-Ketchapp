# Arrow Ketchapp – Unity Project

A simple, arcade-style endless runner where you control an arrow, collect items, and avoid walls.

---

## Project Overview
- **Engine**: Unity (2D)
- **Core Gameplay**:
  - The player controls an arrow that moves forward constantly.
  - Tapping/holding input steers the arrow between two angles.
  - Collectibles extend the arrow's tail and increase score.
  - Gems are a premium currency and trigger color transitions.
  - Hitting a wall ends the run and shows a game-over screen.

---

## Main Scripts

### Player & Tail
- **`Assets/Scripts/Controller.cs`**
  - Handles player movement, steering, and game state–aware updates.
  - Manages acceleration from rest to a target forward speed.
  - Spawns and manages a pool of tail segments.
  - Handles trigger interactions (collectibles, gems, walls, and spawn triggers).

- **`Assets/Scripts/Tail.cs`**
  - Makes each tail segment follow either the player or the previous segment using `SmoothDamp`.
  - Receives initialization from `Controller` (who it follows and its order in the trail).

### World & Camera
- **`Assets/Scripts/EndlessScroller.cs`**
  - Manages two alternating patch parents to create an endless path.
  - Picks random patch variants, resets collectibles and gems, and repositions patches.
  - Handles wall and background color gradients.

- **`Assets/Scripts/CameraScript.cs`**
  - Smoothly follows the player on the Y-axis while in gameplay.

### Game State, Data, Audio
- **`Assets/Scripts/EventsManager.cs`**
  - Central game state and UI manager.
  - Switches between Gameplay and Gameover screens.
  - Resets systems on restart (player, camera, scroller, audio, data).

- **`Assets/Scripts/Gamedata.cs`**
  - Stores persistent data: best score, total gems, and games played count (via `PlayerPrefs`).
  - Tracks the current run score and notifies the UI when it changes.

- **`Assets/Scripts/AudioManager.cs`**
  - Centralized access to sound effects and background music.

### Misc
- **`Assets/Scripts/FPSscript.cs`**
  - Simple on-screen FPS counter.

- **`Assets/SplashManager.cs`**
  - Plays a full-screen splash movie then loads the main scene.

- **Ads / Plugins**
  - `Assets/AdmobTestExamples/*` and `Assets/Plugins/AdmobAPI/*` provide AdMob integration.

---

## How to Run
1. Open the project in **Unity**.
2. Open the main gameplay scene: `Assets/Scenes/main.unity`.
3. Press **Play**.
4. Use your configured input (e.g. a UI button or mouse/touch) wired to:
   - `Controller.OnClickTapDown()` on press/hold.
   - `Controller.OnClickTapUp()` on release.

---

## Configuration Tips

### Controller (Player Arrow)
Found on the player GameObject with `Controller` attached.

Key fields:
- **AngleInDegrees** – Maximum steering angle to each side.
- **ForwardSpeed** – Target forward movement speed.
- **AccelerationTime** – Seconds to accelerate from 0 to `ForwardSpeed`.
- **TurnSpeed** – How quickly the arrow lerps toward the target steering angle.
- **StartPosition** – Where the arrow spawns each run.
- **TailPoolCount** – Number of tail segments pre-instantiated for object pooling.
- **TailInitialSpacing** – Initial spacing between tail segments on spawn.
- **tailPrefab** – Prefab for a tail segment.
- **tailHolder** – Parent transform that holds all tail segments.

### EndlessScroller
- Configure `Patch` array with the two root patch GameObjects.
- Assign `Patch1Prefabs` and `Patch2Prefabs` with your level chunk variants.
- Ensure each patch prefab has `Collectibles` and `Gems` child transforms.
- Assign `WallMaterial` and gradient color arrays for wall/background transitions.

### EventsManager
- Link `Gameplay` and `Gameover` canvases or panels.
- Wire up `Score`, `BestScore`, `TotalGems`, and `gameplayScore` UI Text references.
- Assign `ScoreAnim` (for score punch animation) and `TutorialText`.

### Gamedata
- No scene wiring required beyond having a single `Gamedata` GameObject.
- Uses `PlayerPrefs` keys: `"BestScore"`, `"Gems"`, `"GamePlayedCount"`.

### AudioManager
- Assign appropriate `AudioSource` references for:
  - `Collectible`
  - `Gem`
  - `Gameover`
  - `Button`
  - `BackgroundMusic`

---

## Change Log

### Unreleased (Refactor & Cleanup)

#### Gameplay & Tail Refactors
- **Controller movement parameters**
  - Added configurable fields:
    - `AccelerationTime` – controls how long it takes to reach `ForwardSpeed`.
    - `TurnSpeed` – controls how quickly the arrow rotates toward its target angle.
    - `StartPosition` – defines the initial spawn position instead of hardcoding `(0, -5, 0)`.
    - `TailInitialSpacing` – defines spawn spacing between tail segments instead of a hardcoded `5`.
  - Grouped movement and tail settings under `[Header("Movement")]` and `[Header("Tail / Trail")]` for better Inspector organization.
  - Replaced magic acceleration constant (`ForwardSpeed / 100`) with a calculation based on `AccelerationTime` and `Time.fixedDeltaTime`.

- **Input & steering**
  - Replaced ambiguous `bool_tap` with `isTurningRight` to clarify intent.
  - Centralized steering logic in `ChangeDirection()` using a `targetAngle` (`+AngleInDegrees` or `-AngleInDegrees`) and `TurnSpeed` for interpolation.
  - `OnClickTapDown()` / `OnClickTapUp()` now simply toggle `isTurningRight`, keeping input handling separated from movement math.

- **Initialization**
  - `Init()` now:
    - Resets `CurrentForwardSpeed` and `TailCount`.
    - Resets `isTurningRight`.
    - Uses `StartPosition` and `AngleInDegrees` to place and orient the arrow.

- **Tail pooling and setup**
  - `InstantiateTail()` now:
    - Positions new segments based on `TailInitialSpacing`.
    - Immediately calls `Tail.Initialize(this, order)` so each tail segment knows which controller and order it belongs to.
    - Still adds each tail transform to `tailParts` for runtime following.

#### Tail behavior cleanup
- **Tail.cs**
  - Introduced `Initialize(Controller controller, int order)` to explicitly pass in the owning controller and segment index.
  - Added `followSmoothTime` (renamed from `overTime`) for clearer meaning and easier tuning.
  - Unified follow logic in `Update()`:
    - Chooses a single `target` transform: either the head or the previous tail segment.
    - Rotates to face the target and uses `SmoothDamp` to follow it.
  - Kept a **fallback** in `Start()`:
    - If `Initialize` was not called (e.g., tail placed manually in a scene), it finds the player by tag and infers its order from `Controller.tailParts`.

#### Collision handling separation
- **Controller.cs**
  - Refactored `OnTriggerEnter2D` into small, focused methods:
    - `HandleCollectible(Collider2D other)` – handles score increment, tail growth, particle effect, sound, and deactivating the collectible.
    - `HandleGem(Collider2D other)` – handles gem count, sound, color transition, and deactivation.
    - `HandleWall()` – handles game over: plays sound, calls `Die()`, and triggers `EventsManager.DisplayGameover()`.
    - `HandleSpawnNew()` – calls `EndlessScroller.SpawnNextPrefab()`.
  - Left `Die()` focused on disabling the head and all tail segments.

#### Singleton safety & data bug fix
- Added duplicate-guard patterns to singleton-like managers to avoid multiple instances in a scene:
  - `AudioManager`, `CameraScript`, `Controller`, `EndlessScroller`, `Gamedata`, `EventsManager` now destroy duplicates in `Awake`/`Start` if an instance already exists.
- **Gamedata.cs**
  - Fixed a bug where `AddGems(int Gems)` was saving gem count into the `"BestScore"` PlayerPrefs key. It now correctly saves to `"Gems"`.

#### Game state type safety
- **EventsManager.cs**
  - Changed `CurrentScreen` from an `int` to the strongly-typed `GameState` enum.
  - Updated all comparisons and assignments to use `GameState.Gameplay` / `GameState.Gameover` instead of casting.
  - Updated dependent scripts (`Controller`, `CameraScript`) to compare directly against the enum.

---

## Future Improvements
- Extract a dedicated `PlayerCollisionHandler` component to further decouple movement from interaction logic.
- Replace string-based `FindChild("Collectibles")` / `FindChild("Gems")` in `EndlessScroller` with serialized references for safer scene changes.
- Add unit tests or play mode tests around scoring, gem persistence, and game state transitions.
**A layered framework for AI\-assisted development with consistent standards and workflows\.**
## 🎯 What is Warping?
Warping is a structured approach to working with AI coding assistants \(particularly Warp AI\) that provides:
* **Consistent coding standards** across languages and projects
* **Reproducible workflows** via task\-based automation
* **Self\-improving guidelines** that evolve with your team
* **Hierarchical rule precedence** from general to project\-specific
## 📚 The Layers
Warping uses a layered architecture where more specific rules override general ones:
```warp-runnable-command
user.md          ← Highest precedence (personal preferences)
  ↓
project.md       ← Project-specific rules and workflows
  ↓
python.md        ← Language-specific standards
go.md
  ↓
taskfile.md      ← Tool-specific guidelines
  ↓
main.md          ← General AI guidelines and agent behavior
  ↓
specification.md ← Lowest precedence (project requirements)
```
### 🔧 Core Files
#### **main\.md** \- Foundation
The central AI guidelines document defining:
* General workflow standards \(documentation, filenames, secrets\)
* Agent persona and behavior
* Quality requirements and safety rules
* Self\-improvement mechanisms
**Start here** to understand core principles\.
#### **user\.md** \- Your Preferences
Personal overrides for any rule in the system:
* How to address you
* Personal workflow preferences
* Custom rules that supersede all others
**Customize this** to make warping yours\.
### 🐍 Language Layers
#### **python\.md**
Python\-specific standards:
* Testing: pytest, ≥75% coverage
* Style: ruff, black, isort \(PEP 8\)
* Types: mypy strict mode
* Docs: PEP 257 docstrings
#### **go\.md**
Go\-specific standards:
* Testing: Testify, ≥75% coverage
* Docs: go\.dev/doc/comment
* Patterns: table\-driven tests, interface design
### 🎛️ Tool Layers
#### **taskfile\.md**
Taskfile best practices:
* Task\-centric workflows
* Composable, documented tasks
* Caching and incremental builds
* Shell robustness patterns
### 📦 Project Layer
#### **project\.md**
Project\-specific overrides:
* Tech stack \(CLI/Web/TUI\)
* Project workflows and commands
* Secrets management
* Coverage requirements
#### **specification\.md**
Links to detailed project specifications\.
### 🧠 Learning & Memory
#### **lessons\.md**
Codified learnings from repeated corrections\. The AI can update this autonomously when discovering better approaches\.
#### **ideas\.md**
New concepts and future directions noticed during development\.
#### **suggestions\.md**
AI\-generated improvement suggestions for your projects\.
## 🚀 Getting Started
### 1\. Set Up Your User Preferences
Edit `user.md` to configure personal preferences:
```markdown
# User Preferences
## Name
Address the user as: **YourName**
## Custom Rules
- Your custom preferences here
```
### 2\. Understand the Hierarchy
Rules cascade with precedence:
1. **user\.md** \(highest\) \- your personal overrides
2. **project\.md** \- project\-specific rules
3. **Language files** \(python\.md, go\.md\) \- language standards
4. **Tool files** \(taskfile\.md\) \- tool guidelines
5. **main\.md** \- general AI behavior
6. **specification\.md** \(lowest\) \- requirements
### 3\. Reference in Warp
Upload these files to **Warp Drive** so they're available to AI sessions:
1. Open Warp
2. Access Warp Drive \(notebooks feature\)
3. Upload relevant warping/\*\.md files
4. Reference them in your Warp rules/agent instructions
### 4\. Use in Projects
For each project:
1. Copy or link the warping directory
2. Create/update `project.md` with project\-specific rules
3. Create/update `specification.md` or link to specs
4. Let the AI reference these during development
### 5\. Evolve Over Time
The warping process improves continuously:
* AI updates `lessons.md` when learning better patterns
* AI notes ideas in `ideas.md` for future consideration
* AI suggests improvements in `suggestions.md`
* You update `user.md` with new preferences
* You update language/tool files as standards evolve
## 💡 Key Principles
### Task\-Centric Workflow with Taskfile
**Why Taskfile?**
Warping uses [Taskfile](https://taskfile.dev) as the universal task runner for several reasons:
1. **Makefiles are outdated**: Make syntax is arcane, portability is poor, and tabs vs spaces causes constant friction
2. **Polyglot simplicity**: When working across Python \(make/invoke/poetry scripts\), Go \(make/mage\), Node \(npm scripts/gulp\), etc\., each ecosystem has different conventions\. Taskfile provides one consistent interface
3. **Better than script sprawl**: A `/scripts` directory with dozens of bash files becomes chaotic—hard to discover, hard to document, hard to compose\. Taskfile provides discoverability \(`task --list`\), documentation \(`desc`\), and composition \(`deps`\)
4. **Modern features**: Built\-in file watching, incremental builds via checksums, proper error handling, variable templating, and cross\-platform support
**Usage:**
```warp-runnable-command
task --list        # See available tasks
task check         # Pre-commit checks
task test:coverage # Run coverage
task dev           # Start dev environment
```
### Test\-Driven Development \(TDD\)
Warping embraces TDD as the default development approach:
1. **Write the test first**: Define expected behavior before implementation
2. **Watch it fail**: Confirm the test fails for the right reason
3. **Implement**: Write minimal code to make the test pass
4. **Refactor**: Improve code quality while keeping tests green
5. **Repeat**: Build features incrementally with confidence
**Benefits:**
* Tests become specifications of behavior
* Better API design \(you use the API before implementing it\)
* High coverage naturally \(≥75% is easy when tests come first\)
* Refactoring confidence
* Living documentation
**In Practice:**
```warp-runnable-command
task test          # Run tests in watch mode during development
task test:coverage # Verify ≥75% coverage
task check         # Pre-commit: all quality checks including tests
```
### Quality First
* ≥75% test coverage \(overall \+ per\-module\)
* Always run `task check` before commits
* Run linting, formatting, type checking
* Never claim checks passed without running them
### Spec\-Driven Development \(SDD\)
Before writing any code, warping uses an AI\-assisted specification process:
**The Process:**
1. **Start with make\-spec\.md**: A prompt template for creating specifications
```markdown
I want to build ________ that has the following features:
1. Feature A
2. Feature B
3. Feature C
```
2. **AI Interview**: The AI \(Claude or similar\) asks focused, non\-trivial questions to clarify:
    * Missing decisions and edge cases
    * Implementation details and architecture
    * UX considerations and constraints
    * Dependencies and tradeoffs

   Each question includes numbered options and an "other" choice for custom responses\.
3. **Generate SPECIFICATION\.md**: Once ambiguity is minimized, the AI produces a comprehensive spec with:
    * Clear phases, subphases, and tasks
    * Dependency mappings \(what blocks what\)
    * Parallel work opportunities
    * No code—just the complete plan
4. **Multi\-Agent Development**: The spec enables multiple AI coding agents to work in parallel on independent tasks
**Why SDD?**
* **Clarity before coding**: Catch design issues early
* **Parallelization**: Clear dependencies enable concurrent work
* **Scope management**: Complete spec prevents scope creep
* **Onboarding**: New contributors/agents understand the full picture
* **AI\-friendly**: Structured specs help AI agents stay aligned
**Example**: See `make-spec.md` template in Warp Drive for the interview process
### Convention Over Configuration
* Use Conventional Commits for all commits
* Use hyphens in filenames, not underscores
* Keep secrets in `secrets/` directory
* Keep docs in `docs/`, not project root
### Safety and Reversibility
* Never force\-push without permission
* Assume production impact unless stated
* Prefer small, reversible changes
* Call out risks explicitly
## 📖 Example Workflows
### Starting a New Python Project
1. AI reads: `main.md` → `python.md` → `taskfile.md`
2. AI sets up: pytest, ruff, black, mypy, Taskfile
3. AI configures: ≥75% coverage, PEP standards
4. You customize: `project.md` with project specifics
### Working on an Existing Go Project
1. AI reads: `user.md` → `project.md` → `go.md` → `main.md`
2. AI follows: go\.dev/doc/comment, Testify patterns
3. AI runs: `task check` before suggesting changes
4. AI respects: your user\.md overrides
### Code Review Session
1. AI references quality standards from language file
2. AI runs `task quality` and `task test:coverage`
3. AI checks Conventional Commits compliance
4. AI suggests improvements → adds to `suggestions.md`
## 🔗 Integration with Warp AI
The warping process is designed for Warp AI's rule system:
1. **Upload to Warp Drive**: Keep main\.md and relevant files in Warp Drive
2. **Create Warp Rules**: Reference warping files in your Warp rules
3. **Project\-Specific Rules**: Add `AGENTS.md` or `WARP.md` in project root that references warping
4. **Automatic Context**: Warp AI loads rules automatically when working in your projects
## 📝 Contributing to Warping
As you use warping:
1. **lessons\.md**: AI adds patterns discovered during development
2. **ideas\.md**: AI notes potential improvements
3. **suggestions\.md**: AI records project\-specific suggestions
4. Review these periodically and promote good ideas to main guidelines
## 🎓 Philosophy
Warping embodies:
* **Correctness over convenience**: Optimize for long\-term quality
* **Standards over flexibility**: Consistent patterns across projects
* **Evolution over perfection**: Continuously improve through learning
* **Clarity over cleverness**: Direct, explicit, maintainable code
***
**Next Steps**: Read [main\.md](./main.md) for comprehensive AI guidelines, then customize [user\.md](./user.md) with your preferences\.